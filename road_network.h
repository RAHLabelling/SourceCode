...
class ContractionIndex
{
    ...
    static distance_t get_cut_level_distance(FlatCutIndex a, FlatCutIndex b, size_t cut_level, uint16_t offset = 0);
    static distance_t get_distance(FlatCutIndex a, FlatCutIndex b,uint16_t offset = 0);
...
public:
    ...
    //判断v是不是u祖先,先u后v，判断前面的是不是后面的祖先
    inline bool isAncestor(NodeID v, NodeID u) const {
        // 从 labels 中取出对应节点的 FlatCutIndex（假设它存储了分区 bitvector）
        const FlatCutIndex& ci_u = labels[u].cut_index;
        const FlatCutIndex& ci_v = labels[v].cut_index;

        // 得到 partition bitvector（假设这个 bitvector 表示节点所属的分区信息）
        uint64_t bv_u = *(ci_u.partition_bitvector());
        uint64_t bv_v = *(ci_v.partition_bitvector());

        // 调用 PBV 中的 is_ancestor 来判断：是否 bv_v 是 bv_u（或相反）的祖先
        // 这里假设 is_ancestor(bv_v, bv_u) 返回 true 表示 v 是 u 的祖先
        return PBV::is_ancestor(bv_v, bv_u);
    }
    ...
    distance_t get_distance(NodeID v, NodeID w,uint16_t offset=0) const;
    distance_t test_time(NodeID v, NodeID w, uint16_t offset = 0) const;
...
private:
    ...
    distance_t distances[33],dist[33];
    NodeID time[33],vistime[33],visstime[33],disttime[33],isbdtime[33];
    uint16_t viss[33];
    bool vis[33];
#endif
#ifdef CONTRACT2D
...
class Graph
{
    ...
    void get_cut(int id,int level, const ContractionIndex& con_index, std::vector<NodeID> upd,std::vector<NodeID>edge, const std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t, distance_t>>>& E,std::vector<NodeID> &ans);
    void get_cutdis(size_t start, size_t end, std::vector<NodeID>& C, std::vector<NodeID>& CALL, const ContractionIndex& con_index, NodeID u, NodeID v, distance_t w,NodeID &ans);
    void get_cutdis2(size_t start, size_t end, std::vector<NodeID>& C, std::vector<NodeID>& CALL, const ContractionIndex& con_index, NodeID u, NodeID v, distance_t w, NodeID& ans);
    ...
    bool run_dijkstra2(NodeID v,NodeID u);
    void dfs(NodeID id);
    ...
    void add_shortcuts(const std::vector<NodeID> &cut, std::vector<CutIndex> &ci,std::vector<NodeID> &border);
    ...
    static void extend_on_partition(std::vector<CutIndex> &ci, double balance, uint8_t cut_level, const std::vector<NodeID> &p, const std::vector<NodeID> &cut,int pid);
    // recursively decompose graph and extend cut index
    void extend_cut_index(std::vector<CutIndex> &ci, double balance, uint8_t cut_level,const std::vector<NodeID> &border,int pid);
...
public:
    ...
    void init(const ContractionIndex& con_index);
    ...
    void process_nodes(size_t start, size_t end, const std::vector<NodeID>& nodes, ContractionIndex& con_index,
        NodeID u, NodeID v, distance_t w, distance_t before_w, const std::vector<CutIndex>& ci, const std::unordered_map<NodeID, distance_t>& get_dis, distance_t md, std::vector<NodeID>& upd,std:: vector<std::pair<std::pair<NodeID, NodeID>, distance_t>> &change,NodeID etime);
    void update_down_par(size_t start, size_t end, const std::vector<NodeID>& nodes, const ContractionIndex& con_index, const std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t, distance_t>>>& edge);
    void change_remain(size_t start, size_t end, std::vector<NodeID>& cutt, ContractionIndex& con_index, const std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t, distance_t>>>& edge, const std::vector<CutIndex>& ci,NodeID tid);
    void update_up_par(size_t start, size_t end, const std::vector<NodeID>& nodes, ContractionIndex& con_index, const std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t,distance_t>>>& edge, const std::vector<CutIndex>& ci,std::vector<NodeID> &nd_cg);
    ...
    void run_dij_upd_down(ContractionIndex& con_index, const std::vector<CutIndex>& ci, const std::vector<NodeID>& bor, NodeID a, NodeID b, distance_t w,NodeID cut, std::vector<std::pair<std::pair<NodeID, NodeID>, distance_t>> &change);
    void change_remain_one(size_t start, size_t end, std::vector<NodeID>& cutt, ContractionIndex& con_index, distance_t w, distance_t bfw, NodeID a, NodeID b, const std::vector<CutIndex>& ci, std::vector<std::pair<std::pair<NodeID, NodeID>, distance_t>> &change, const std::unordered_map<NodeID, distance_t>& get_dis, distance_t md,NodeID tid,NodeID etime);
    void change_remain_one2(size_t start, size_t end, std::vector<NodeID>& cutt, ContractionIndex& con_index, distance_t w, distance_t bfw, NodeID a, NodeID b, const std::vector<CutIndex>& ci, std::vector<std::pair<std::pair<NodeID, NodeID>, distance_t>>& change, const std::unordered_map<NodeID, distance_t>& get_dis, distance_t md,NodeID tid,NodeID etime);
    void run_dij_upd_up(const ContractionIndex& con_index, const std::vector<CutIndex>& ci, NodeID a, NodeID b, distance_t bfw,distance_t w, NodeID cut);
    void Contract(ContractionIndex& con_index, NodeID s, distance_t d);
    void update_many_edge_up(const std::vector<CutIndex>& ci, ContractionIndex& con_index, std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t,distance_t>>>& edge);
    void update_many_edge_down(const std::vector<CutIndex>& ci, ContractionIndex& con_index, std::vector<std::pair<std::pair<NodeID, NodeID>, std::pair<distance_t, distance_t>>>& edge);
    void update_edge(const std::vector<CutIndex>& ci,ContractionIndex &con_index, NodeID u, NodeID v, distance_t w,NodeID etime);//update the weight of the edges
    void find_affected_point(const ContractionIndex &con_index, distance_t &get_dis, NodeID u, NodeID v, distance_t w, distance_t bfw,NodeID etime);
