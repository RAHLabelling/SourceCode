...
/*
void run_dijkstra_llsub_par_upd(const vector<CutIndex> &ci,const std::vector<NodeID>& vertices)
{
    CHECK_CONSISTENT;
...
void Graph::update_edge(const vector<CutIndex>& ci, ContractionIndex &con_index, NodeID u, NodeID v, distance_t w, NodeID etime) {
    ...
    // submit find_affected_point as fire-and-forget (same semantics as detached thread)
    {
        ...
        (void)pool.submit(&Graph::find_affected_point, this, std::cref(con_index), std::ref(md), u, v, w, before_w, etime);
    }
...
void get_edge(istream& in, vector < pair<pair<NodeID, NodeID>, pair<distance_t, distance_t>>>& edge, size_t fg, const ContractionIndex& con_index)
{
    char line_id;
    uint32_t v, w, d;
    vector<NodeID> rd;

    // ʹ�� std::random_device ���ɸ��õ�����
    std::random_device rd_device;
    std::mt19937 rng(rd_device()); // ʹ��Mersenne Twister����

    std::uniform_int_distribution<> dist; // ������ȷֲ���������

    vector < pair<pair<NodeID, NodeID>, pair<distance_t, distance_t>>> edge2;
    ...
    cout << edge2.size() << endl;
    for (int i = 1; i <= 1000; i++) {
        edge.push_back(edge2[dist(rng) % (edge2.size())]);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for (int f = file_start; f < argc; f++)
    {
        ...
        for (size_t i = 0; i < repeats; i++)
        {
            ...
            fstream ft(filename);
            get_edge(ft, edge,1,con_index);//1������up��-1������down
            ...
            for (auto e : edge) {
                ...
                if (con_index.is_contracted(u) || con_index.is_contracted(v)) {
                    if (a.distance_offset > b.distance_offset)
                        g.Contract(con_index, u, b.distance_offset + w - before_w);
                    else if (a.distance_offset < b.distance_offset)
                        g.Contract(con_index, v, a.distance_offset + w - before_w);
                    continue;
                }
                edge2.pb(e);
            }
           ...
           for (int i = 1; i <= 1000; i++) {
            ...
            for (pair<NodeID,NodeID> q : queries)
                con_index.test_time(q.first, q.second);
